...
void CMario::Update(DWORD dt, vector<LPGAMEOBJECT> *coObjects)
{
	...
	if (x <= 0 || x >= BackBufferWidth - MARIO_WIDTH || y <= 0 || y >= BackBufferWidth - MARIO_WIDTH) {
		...
		if (dynamic_cast<CPlayScene*>(CGame::GetInstance()
			->GetCurrentScene())->getObjectSize() < 8) {
		...
		DebugOut(L"[INFO] sprite random vy : %f\n", y);
	...
	//DebugOut(L"[INFO] sprite random x : %f\n", x);
	//DebugOut(L"[INFO] sprite random y : %f\n", y);
	//DebugOut(L"[INFO] sprite random vx : %f\n", vx);
	//DebugOut(L"[INFO] sprite random vy : %f\n", vy);
	// Simple fall down
	//vy += MARIO_GRAVITY*dt;
...
void CMario::SetState(int state)
{
	...
	case MARIO_STATE_WALKING_RIGHT:
		...
		vy = 0;
		DebugOut(L"ccccccccccc \n");
		break;
	case MARIO_STATE_WALKING_LEFT: 
		vx = -MARIO_WALKING_SPEED;
		vy = 0;
		DebugOut(L"aaaaaaaaaaaaaaaaaaaaa \n");
		break;
